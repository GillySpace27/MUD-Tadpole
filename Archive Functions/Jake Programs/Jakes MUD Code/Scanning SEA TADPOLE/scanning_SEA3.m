function [Elam,tau,w_eq] = scanning_SEA3(s,vid,Ref1,bg,nop,range,dl,axis_num,home1,lam0,pks)
%% options 
...
% the counter for the file saving is:
k=0;
% the maximum array size before saving is:
max_array=1000;
...
%     hold_on(6); %range = 22 and 100 pts
    ...
    % Finding the phase and the spectrum:
        ...
        Elam(:,n-k*max_array)=sqrt(sum(abs(cut),2).^2./sum(Ref1,2)).*exp(i*mean(unwrap(angle(cut(:,2:end-1))),2));
     if n-k*max_array==max_array
         savefile=cat(2,num2str(k+1),'mud_trace_file.mat');
         save(savefile,'Elam');
         clear Elam
         k=k+1;
     end
