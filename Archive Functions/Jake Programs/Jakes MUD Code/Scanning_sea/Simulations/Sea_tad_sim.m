...
[phi,spectrum]= find_phi_jake2(rot90(I1-Ref1),rot90((Ref1)),dl*1e9);
...
hold;
plot(lam*1e9,Norm(abs(Ew).^2),'r');
spec_plot1('Comparison of the retrieved (red) and expected (blue) spectrum');
hold;
...
%subplot(2,1,2);
figure
plot(t*1e-3,Norm(abs(Ut).^2),'.');
hold;
plot(t*1e-3,Norm(abs(fftc(Ew)).^2),'r');
[zeroed_data]=sub_bg(abs(Ew),100);
a=zeroed_data.*exp(i*angle(Ew));
plot(t*1e-3,Norm(abs(fftc(a)).^2),'g');
temp_plot1('Temporal profile retrieved (red) and expected (blue)','Intensity');
hold;