...
function [Et,E_lam,t_f,lam_eq,amp,t1]=best_concat2_mud(Ew,tau,w,lam0)
c=300;
% Generating the time window:
[tw,delta_t]=time_window(w,tau);
% Filtering out only retrieved regions in the time window:
[Valid_unk,Valid_H,x]=temporal_filter_sea_single_mud(fftc(Ew,[],1),tw,size(Ew,2));
...
% indexidbquitng the retrieved amplitudes
...
%[Et]=new_concat2_no_rephase_fields(Valid_unk,Valid_H,tw,x);
%[Et]=new_concat2_no_rephase(Valid_unk,Valid_H,tw,x,offset);
% making the time axis:
Nt=length(Et);
[t_f]=(-Nt/2*delta_t:delta_t:(Nt-1)/2*delta_t);
w_f=ttow(t_f,2*pi*c/lam0);
% filtering out the high freq's:
%Et=high_freq_filter(Et,[200 800]);
