...
Unk=Unk1.*(exp(-i*kk.*xx*.06));%.*exp(-1e-12*i*kk.*ww*p*d/f);%.*exp(0*-10e-12*i*xx.*ww).*exp(0*1e-4*i*ww.^3*GDD.^(3/2)).*exp(.1*i*kk.*xx.^2);
...
fint_1d = norm1(sum(abs(fftc(I2-Ref-I0,[],2)),2));
[yy1,xx1] = findpeaks(fint_1d);
[yyy1,xxx1]= findpeaks(yy1);
x1 = xx1(xxx1);
points = x1;
%points=[x1(1),x1(2)];
dp=round(abs(x1(1)-x1(2))/4);
[cut]=filter_snm(fftc(I2-Ref-I0,[],2),points,dp);
